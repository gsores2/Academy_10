...
namespace Academy.lambda
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            FilterInt filterOdd = new FilterInt(IsOdd); // costruisco il filtro che è un delegate di tipo FiòterInt (pposso cambiare il metodo come voglio, solo aggiungendo un metodo con firma uguale al delegatre)
            //List<int> result = FilterInts(lst, filterOdd); // risultato del filtraggio
            // oppure List <int> result= FilterInts(lst, IsEven); // passo direttamente il metodo e lui fa il delegate
            // oppure lambda expression (che è un delegate sintatticamente
